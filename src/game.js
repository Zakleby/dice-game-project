...
async function game() {
    ...
    infoLog(`This game is ${loggedIn[0].username} VS ${loggedIn[1].username}\n\n`)

    for (let i = 1; i < 11; i++) {
        console.log(`${i}, ${i%2}`)
        prompt(chalk.magentaBright('[Waiting..]') + ` Waiting for ${loggedIn[i].username}... Press ENTER to roll the dice`)
        await roll(i%2)
    }
}
